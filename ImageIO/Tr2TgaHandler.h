...
public:
	...
	bool Save( const Tr2HostBitmap& image, IBlueStream* output );
	bool GenerateMips();

	static bool IsSaveSupported( Tr2RenderContextEnum::PixelFormat format );
	static bool SaveHeader( uint32_t width, uint32_t height, Tr2RenderContextEnum::PixelFormat format, IBlueStream* output );
	static bool SaveRows( uint32_t width, uint32_t height, Tr2RenderContextEnum::PixelFormat format, const void* data, IBlueStream* output );
...
private:
	...
	bool ReadPalette( unsigned char*& palette );
	bool ReadRawData( unsigned char* palette );
	bool ReadRLEData( unsigned char* palette );	
