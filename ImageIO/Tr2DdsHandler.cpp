...
#define TR2DDSLOADER_DEBUGGING 0
#if TR2DDSLOADER_DEBUGGING
...
bool Tr2DdsHandler::ReadHeader()
{
	CCP_STATS_ZONE( __FUNCTION__ );

	if( !m_source || m_source->Read( &m_header, sizeof( m_header ) ) == -1 )
	{
		return false;
	}
	...
	if( !IsCubeTexture() )
	{
		...
		if( skipCount )
		{
			...
			m_source->Seek( skipBytes, BS_CURRENT );
...
bool Tr2DdsHandler::ReadImage()
{
	...
	if( FindDdsFormat( m_header.ddspf ).first == DDSFMT_A8L8 )
	{
...
bool Tr2DdsHandler::IsSupported()
{
...
bool Tr2DdsHandler::Save( const Tr2HostBitmap& image, IBlueStream* output )
{
	CCP_ASSERT( output );
