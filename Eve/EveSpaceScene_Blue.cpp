...
const Be::ClassInfo* EveSpaceScene::ExposeToBlue()
{
    EXPOSURE_BEGIN( EveSpaceScene, "" )
		...
		MAP_ATTRIBUTE( 
			"nebulaIntensity", 
			m_nebulaIntensity, 
			"How intense is the nebula, only used when there is no reflection probe and nebulaBrightnessOverride is 0\n"
			":jessica-group: Lighting", 
			Be::READWRITE | Be::PERSIST )
		...
		( 
			...
			"Nebula brightness intensity override (including reflection). Set to 0 to disable. Is not used if we have a reflection probe\n"
			":jessica-group: Lighting",
			Be::READWRITE
		)
		...
		MAP_ATTRIBUTE(
			...
			m_reflectionProbeIntensity,
			"Determines how much we scale the cube map generated by the reflection probe\n"
			":jessica-group: Lighting",
			Be::READWRITE | Be::PERSIST | Be::NOTIFY )

		MAP_ATTRIBUTE(
			"reflectionProbeBackLightingContrast",
			m_reflectionProbeBackLightingContrast,
			"Reflection probe back light contrast\n"
			":jessica-group: Lighting",
			Be::READWRITE | Be::PERSIST | Be::NOTIFY )	

		MAP_ATTRIBUTE(
			"reflectionProbeBackLightingColor",
			m_reflectionProbeBackLightingColor,
