#include "StdAfx.h"

#if APEX_ENABLED

#include "Tr2TurbulenceFSActor.h"
#if USE_APEX_FIELD_SAMPLER
#include "Tr2ApexRes.h"
#include "TriSettingsRegistrar.h"
#include "blue/include/IBlueResMan.h"
#include "Resources/TriGrannyRes.h"

#include "Apex.h"

#include "NxTurbulenceFSActor.h"
#include "NxTurbulenceFSAsset.h"
#include "NxParamUtils.h"

static bool s_apexHardwareClothEnabled = true;
TRI_REGISTER_SETTING( "apexHardwareClothEnabled", s_apexHardwareClothEnabled );

Tr2TurbulenceFSActor::Tr2TurbulenceFSActor( IRoot* lockobj ) :
	m_turbulenceFSActor( NULL ),
	m_isInScene( false ),
	m_maxDistance( 100.0f ),
	m_distanceWeight( 1.0f ),
	m_bias( 0.0f ),
	m_benefitBias( 0.0f )
{
}

Tr2TurbulenceFSActor::~Tr2TurbulenceFSActor()
{
	Cleanup();

	if( m_apexRes )
	{
		m_apexRes->RemoveNotifyTarget( this );
	}

}

bool Tr2TurbulenceFSActor::Initialize()
{
	Cleanup();

	if( m_apexRes )
	{
		m_apexRes->RemoveNotifyTarget( this );
	}

	IBlueResource* p = BeResMan->GetResource( m_resPath, "apex" );
	m_apexRes.Attach( dynamic_cast<Tr2ApexRes*>( p ) );

	if( m_apexRes )
	{
		m_apexRes->AddNotifyTarget( this );
	}	

	return true;
}

void Tr2TurbulenceFSActor::AddToApexScene()
{
	m_isInScene = true;

	if( !m_apexRes )
	{
		// No turbulenceFS resource, can't create the actor.
		return;
	}

	if( m_apexRes->IsLoading() )
	{
		// Not finished loading - we'll come back once it is.
		return;
	}

	physx::apex::NxApexAsset* asset = m_apexRes->GetAsset();
	if( !asset )
	{
		CCP_LOGERR( "Tr2TurbulenceFSActor::AddToApexScene: %S has no turbulenceFS asset", m_apexRes->GetPath() );
		return;
	}

	Cleanup();

	NxParameterized::Interface* actorDesc = asset->getDefaultActorDesc();
	CCP_ASSERT( actorDesc != NULL );

	{
		#define VERIFY_PARAM(_A) { NxParameterized::ErrorType error = _A; CCP_ASSERT(error == NxParameterized::ERROR_NONE); }

		NxParameterized::Handle actorHandle(*actorDesc);

		// No util method for this
	}


	// create the actor
	physx::apex::NxApexActor* apexActor = asset->createApexActor( *actorDesc, *g_Tr2Apex->GetApexScene() );
	CCP_ASSERT( apexActor );

	m_turbulenceFSActor = static_cast<physx::apex::NxTurbulenceFSActor*>(apexActor);
}

void Tr2TurbulenceFSActor::RemoveFromApexScene()
{
	m_isInScene = false;
	Cleanup();
}

void Tr2TurbulenceFSActor::ReleaseCachedData( BlueAsyncRes* p )
{
	Cleanup();
}

void Tr2TurbulenceFSActor::RebuildCachedData( BlueAsyncRes* p )
{
	if( m_isInScene )
	{
		AddToApexScene();
	}
}

const std::string& Tr2TurbulenceFSActor::GetResPath() const
{
	return m_resPath;
}

void Tr2TurbulenceFSActor::SetResPath( const std::string& val )
{
	m_resPath = val;
	Initialize();
}

void Tr2TurbulenceFSActor::Cleanup()
{
	if( m_turbulenceFSActor )
	{
		if( g_Tr2Apex->ApexIsClothSimInProgress() )
		{
			g_Tr2Apex->ApexDelayReleaseActor( m_turbulenceFSActor );
		}
		else
		{
			m_turbulenceFSActor->release();
		}
		m_turbulenceFSActor = NULL;
	}

}

void Tr2TurbulenceFSActor::SetVisualize( bool b )
{
	CCP_ASSERT(m_turbulenceFSActor);
	if ( m_turbulenceFSActor )
	{
		// TODO:JWR
	}
}

bool Tr2TurbulenceFSActor::GetVisualize()
{
	bool ret = false;
	CCP_ASSERT(m_turbulenceFSActor);
	if ( m_turbulenceFSActor )
	{
		// TODO:JWR
	}
	return ret;
}


#endif

#endif